...
-- 1. BACKUP
-- ------------------------------------------------------------
DROP TABLE IF EXISTS _undo_geo_import_20260803;

CREATE TABLE _undo_geo_import_20260803 AS
SELECT
  'institucion' AS tipo,
  i.id AS registro_id,
  i.nombre AS nombre,
...
CREATE INDEX ON _undo_geo_import_20260803(tipo, registro_id);

RAISE NOTICE 'Backup creado: % filas', (SELECT count(*) FROM _undo_geo_import_20260803);

-- ------------------------------------------------------------
-- 2. INSTITUCIONES IMPORTADAS SIN REFERENCIAS -> ELIMINAR
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 3. MUNICIPIOS IMPORTADOS SIN REFERENCIAS -> ELIMINAR
-- ------------------------------------------------------------
...
WHERE DATE(m.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota') = '2026-08-03'
  ...
  AND NOT EXISTS (SELECT 1 FROM public.fichas_rlt f WHERE f.municipio = m.nombre OR f.comuna_barrio = m.nombre)
  AND NOT EXISTS (SELECT 1 FROM public.ae_rectores_2025 r WHERE r.entidad_territorial = m.nombre);
...
-- ------------------------------------------------------------
-- 4. ENTIDADES TERRITORIALES IMPORTADAS SIN REFERENCIAS -> ELIMINAR
-- ------------------------------------------------------------
...
-- ============================================================
-- UNDO (ejecutar solo si se quiere revertir el rollback)
-- ============================================================
/*
BEGIN;
...
FROM _undo_geo_import_20260803
WHERE tipo = 'entidad'
