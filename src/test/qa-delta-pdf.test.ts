...
async function writePdf(cohorteNombre: string, path: string) {
...
describe("QA - PDF cover", () => {
  it("writes long-cohorte PDF", async () => {
    await writePdf("Rionegro 2025, Itagüí 2025, Medellín 2025, Oriente 2026, Quibdó 2026", "/tmp/qa-delta-long.pdf");
  });
  it("writes single-cohorte PDF", async () => {
    await writePdf("Rionegro 2025", "/tmp/qa-delta-single.pdf");
  });
});
