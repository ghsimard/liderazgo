...
serve(async (req) => {
  ...
  try {
    const { sectionType, sectionTitle, chartData, generalStats, filterType, filterModule, filterRegion, totalResponses, comments, overallSatisfaction, cohorteNombre, maxScore, cohortIni, cohortEvo, cohortDelta, deltasPorGrupo } = await req.json();
